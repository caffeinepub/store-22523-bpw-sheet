...
export default function BPWSheet() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        if (actor) {
          // Load product names and current sheet in parallel (cache-first, instant)
          const [names, allSheets, loadedSheet] = await Promise.all([
        ...
        // After loading, check for pending writes to determine status
        if (!cancelled) {
          setSyncStatus("live");
        }
      } catch (err) {
        ...
        // Keep status as "syncing" so the dot stays yellow (retrying), not red/orange
        if (!cancelled) {
          setSyncStatus("syncing");
        }
      }
  ...
  // Guarded save with auto-retry: sets isSavingRef around every canister write so live polling
  // doesn't overwrite a save that is currently in flight.
  // Retries up to 5 times with 2-second delay before throwing.
    ...
    async (
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          ...
          lastErr = err;
          console.warn(`Save attempt ${attempt + 1} failed, retrying...`, err);
      ...
      isSavingRef.current = false;
      throw lastErr; // only throw after all retries exhausted
    },
  );
  ...
  const handleProductNameChange = useCallback(
    async (idx: number, newName: string) => {
      ...
      try {
        ...
        setSyncStatus("syncing");
        toast.warning(
          "Could not reach server. Your entry is kept and will sync automatically.",
        );
      }
      setSheet((prev) => {
        if (!prev) return prev;
  ...
  const handleDefaultQtySet = useCallback(async () => {
    ...
    try {
      prev = actor
        ? await getMostRecentLockedSheetFromBackend(actor, selectedDate)
        : null;
    } catch (err) {
